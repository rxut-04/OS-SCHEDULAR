...
export function priorityScheduling(processes: Process[]): SchedulingResult {
  ...
  while (remainingProcesses.length > 0) {
    ...
    const readyQueue = availableProcesses.map(p => `${p.id}(P${p.priority})`);

    steps.push({
      time: currentTime,
      action: `Execute ${selectedProcess.id}`,
      processId: selectedProcess.id,
      reason: `${selectedProcess.id} selected - has highest priority (${selectedProcess.priority}) among ready processes: ${readyQueue.join(', ')}. (Lower number = Higher priority)`,
      readyQueue: availableProcesses.filter(p => p.id !== selectedProcess.id).map(p => p.id),
    });
