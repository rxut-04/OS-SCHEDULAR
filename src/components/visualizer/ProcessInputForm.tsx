    ...
    <div className="glass-effect rounded-xl p-6 animate-fade-in">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="text-sm px-3 py-1.5 rounded-lg bg-[var(--bg-tertiary)] text-[var(--text-secondary)] hover:text-[var(--accent-primary)] transition-colors"
        >
          Load Sample
        </button>
      </div>
      ...
      <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
        {processes.map((process, index) => (
          <div 
            key={index}
            className="flex items-center gap-2 p-3 rounded-lg bg-[var(--bg-tertiary)] animate-slide-in"
          ...
          >
            ...
            <input
              ...
              placeholder="ID"
              className="w-16 px-2 py-1.5 rounded bg-[var(--bg-secondary)] border border-[var(--border-color)] text-sm font-mono focus:outline-none focus:border-[var(--accent-primary)] text-[var(--text-primary)]"
            />

            <div className="flex-1 grid grid-cols-2 gap-2" style={{ gridTemplateColumns: showPriority ? 'repeat(3, 1fr)' : 'repeat(2, 1fr)' }}>
              <div>
                <label className="text-xs text-[var(--text-muted)] block mb-1">AT</label>
                <input
                  type="number"
                  min="0"
                  value={process.arrivalTime}
                  onChange={(e) => updateProcess(index, 'arrivalTime', e.target.value)}
                  className="w-full px-2 py-1.5 rounded bg-[var(--bg-secondary)] border border-[var(--border-color)] text-sm font-mono focus:outline-none focus:border-[var(--accent-primary)] text-[var(--text-primary)]"
                />
              </div>
              <div>
                <label className="text-xs text-[var(--text-muted)] block mb-1">BT</label>
                <input
                  type="number"
                  min="1"
              ...
              {showPriority && (
                <div>
                  <label className="text-xs text-[var(--text-muted)] block mb-1">Priority</label>
                  <input
                    type="number"
                    min="1"
            ...
            <button
              ...
              className="p-1.5 rounded hover:bg-[var(--error)]/20 text-[var(--text-muted)] hover:text-[var(--error)] disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            >
      ...
      <button
        onClick={addProcess}
        className="w-full mt-3 py-2 rounded-lg border border-dashed border-[var(--border-color)] text-[var(--text-muted)] hover:border-[var(--accent-primary)] hover:text-[var(--accent-primary)] transition-colors flex items-center justify-center gap-2"
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {showTimeQuantum && (
        <div className="mt-4 p-3 rounded-lg bg-[var(--bg-tertiary)]">
          ...
          <input
            ...
            className="w-full px-3 py-2 rounded bg-[var(--bg-secondary)] border border-[var(--border-color)] font-mono focus:outline-none focus:border-[var(--accent-primary)] text-[var(--text-primary)]"
          />
        </div>
      )}
      ...
      <button
        ...
        className="w-full mt-4 py-3 rounded-lg bg-[var(--accent-primary)] text-[var(--bg-primary)] font-semibold hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
      >
